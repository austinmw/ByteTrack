...
def last_digit_padded(frame_path):
    pathname = os.path.splitext(frame_path)[0]
    try:
        last_digits = re.compile(r'(\d+)$').search(pathname).group(1)
    except Exception as e:
        raise ValueError(f'pathname "{pathname}" does not end with digits! Exception: {e}')
    return last_digits.zfill(10)


class VideoSortedFolderReader(VideoBaseClass):
    def __init__(self, data_path, fps, extensions=['png', 'jpg', 'jpeg']):
        self._data_path = data_path
        self.extensions = extensions
        frame_paths = []
        [frame_paths.extend(glob.glob(os.path.join(data_path, f'*.{e}'))) for e in self.extensions]
        self._frame_paths = sorted(frame_paths, key=last_digit_padded)
        self._next_frame_to_read = 0
