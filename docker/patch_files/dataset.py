...
class AnnoEntity:
    ...
    def __init__(self, time=None, id=None, raw_info=None, validate=True):
        ...
        if raw_info is not None:
            if time is not None or id is not None:
                logger.warning("time or id were specified, but raw_info was set and so they will be ignored")
            self._parse_info()
        else:
            if validate:
                self.time = time
    ...
    def labels(self, new_labels):
        if self._labels:
            ...
            logger.warning("updating the labels with different keys: {}".format(diff_keys))
    ...
    def blob(self, new_blob):
        if self._blob:
            ...
            logger.warning("updating the blob with different keys: {}".format(diff_keys))
        self._blob = new_blob
...
class DataSample:
    ...
    def metadata(self, new_md):
        if self._metadata:
            ...
            logger.warning("updating the metadata with different keys: {}".format(diff_keys))
    ...
    def _lazy_load(self):
        ...
        if self.metadata and self._raw_info.get(FieldNames.METADATA) != self.metadata:
            logger.info("metadata did not match lazy loaded value for sample: {}, ignoring loaded value, this should be"
    ...
    def _dump_for_lazy_load(self):
        if not self._lazy_loaded:
            logger.debug("nothing lazy loaded to dump, returning")
            return
        filepath = self._get_lazy_load_path()
    ...
    def clear_lazy_loaded(self, clear_metadata=False):
        if not self._lazy_loaded:
            logger.debug("nothing lazy loaded so nothing to clear")
            return
        self._entities = []
        self._raw_info = None
        self._init_entity_fields()
...
class GluonCVMotionDataset:
    ...
    def __init__(self, annotation_file=None, root_path=None, split_file=None, load_anno=True):
        ...
        if annotation_file is None:
            ...
            logger.info("Annotation file not provided, defaulting to '{}'".format(annotation_file))
        ...
        if self._root_path:
            ...
            logger.warning('Root path was not set for dataset, this should only happen when loading a lone annotation'
                        ' file for inspection')
        ...
        if load_anno:
            if os.path.exists(self._anno_path):
                logger.info('Loading annotation file {}...'.format(self._anno_path))
                # load annotation file
                if self._get_pickle_path().exists():
                    logger.info('Found pickle file, loading this instead')
            ...
            logger.info('Skipping loading for annotation file {}'.format(self._anno_path))
            self._split_path = self._get_split_path(split_file, self._anno_path)
    ...
    def _get_root_path(self, root_path, annotation_file):
        if root_path is None:
            ...
            if self.ANNO_DIR in dirpath:
                ...
                logger.info("Dataset root path inferred to be: {}".format(root_path))
        return root_path
    ...
    def _get_split_path(self, split_file, anno_path):
        ...
        if not os.path.isabs(split_path):
            ...
            logger.info("Split subpath: {}".format(split_subpath))
        return split_path
    ...
    def get_split_ids(self, splits=None):
        ...
        for split in splits:
            if split not in self._splits:
                logger.warning("Provided split: {} was not in dataset".format(split))
            else:
    ...
    def get_split_samples(self, splits=None):
        ...
        for split_id in split_ids:
            if split_id in self._samples:
                ...
                logger.info(f"Dataset is missing sample: {split_id} in split {self.get_use_for_id(split_id)}, skipping")
        return samples
    ...
    def _load_pickle(self, filepath=None):
        ...
        if not os.path.exists(filepath):
            return False
        logger.info('Loading pickle file {}'.format(filepath))
        try:
            loaded_dict = load_pickle(filepath)
        except OSError:
            logger.warning("Failed to load pickle")
            return False
        ...
        if stored_time == modified_time:
            ...
            logger.info(('The pickle stored modification time did not match the annotation, so not loading,'
    ...
    def _parse_anno(self, annotation_file):
        ...
        logger.info("loaded {} samples".format(len(self._samples)))

    def _load_split(self):
        if not os.path.exists(self._split_path):
            logger.warning("Split path {} not found, skipping loading".format(self._split_path))
            return
        self._splits = load_json(self._split_path)
        split_sample_nums = {k: len(v) for k, v in self._splits.items()}
        logger.info("Loaded splits with # samples: {}".format(split_sample_nums))
